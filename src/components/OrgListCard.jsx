...
export default function RecipeReviewCard(props) {
  ...
  const deleteApp = () => {
    callApi(`/usersrvc/api/org/${org.id}`, 'DELETE')
      .then(e => { if (e.success) { showSuccess('Group has been deleted!'); props.onUpdate(); handleModalClose() } })
  }
    ...
    <div>
      <div style={modalStyle} className={classes.paper}>
        <div style={{ display: 'block' }}>
          ...
          <div style={{ paddingBottom: '10px' }}><span className={classes.modalheader}>Delete {org.name} Organisation</span></div>
        </div>
        <div className={classes.content}>
          ...
          <Button style={{ backgroundColor: '#C53230', borderRadius: '5px', margin: '10px' }}><span style={{ color: 'white' }} onClick={deleteApp}>Delete</span></Button>
      ...
      <Grid container spacing={3}>
        <Grid item xs={5} onClick={props.onClick}>
          <div className={classes.flexdiv}>
            <div className={classes.headdiv}>
              ...
              <span className={classes.designation}> {org.description ? ellipsis(org.description, 50) : ""  || "No Description"} </span>
            </div>
          </div>
        </Grid>
        <Grid item xs={4} className={classes.griditemone} onClick={props.onClick}>
          <div className={classes.divone}>
            ...
            <span className={classes.ephone}> {org.numberOfUsers} </span>
          </div>
          <div className={classes.divone}>
            <DomainIcon className={classes.icon} />
            <span className={classes.ephone}> {org.numberOfApps} </span>
          </div>
        </Grid>
        <Grid item xs={3} className={classes.griditemtwo}>
          ...
          <Link to={`/dash/directory/orgs/${org.id}`}>
          ...
          {/* {isActiveForRoles(['ORG_ADMIN', 'DOMAIN_ADMIN']) && ( */}
          <IconButton className={classes.iconbuttontwo} onClick={handleModalOpen}>
