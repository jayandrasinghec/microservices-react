...
import { PutManualStart, PutManualStop} from '../../actions/CampaignActions';
...
export default function ConfirmationModal(props) {
  const {modalData, setModalData, name, filters, onClose} = props;
  const classes = useStyles();
  const dispatch = useDispatch();
  const [modalStyle] = React.useState(getModalStyle);

  const handleConfirm = () => {
    modalData.running ? dispatch(PutManualStop(modalData.id, filters)) : dispatch(PutManualStart(modalData.id, filters))
    // setModalData(null)
    onClose()
  }
