...
export default function AddRoleButton(props) {
  ...
  const isError = errors && errors.name === null && errors.description === null && errors.id === null
  ...
  const checkId = () => {
    if(newData.roleId === ""){
      setError({ id: 'Role Id is required' })
    }
    else if(props.data.length && newData.roleId){
       props.data.forEach(item=>{
        if(item.roleId.toLowerCase() === newData.roleId.toLowerCase()){
            setError({ id: 'Role Id already exists' })
        }else{
          setError({ id: null })
        }
      });      
  ...
  const addbody = (
    ...
    disabled={!isValid || !isError || saving}
    saving={saving}
    onSubmit={onSubmit}
    body={
      <form>
